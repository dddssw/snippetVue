...
import {infoData} from './info/skill'
import { devProblem } from "./info/dev";
import {
  rawlist,
  input,
  checkbox,
...
program
  .command("dev")
  .description("信息")
  .argument("<string>", "查询什么(步骤之类的)")
  .action(async (str, options) => {
    if (str in devProblem) {
      devProblem[str]();
    } else {
      const keys = Object.keys(devProblem).filter((item) => item.includes(str));
      let res = [];
      keys.forEach((item, index) => {
        const step = minDistance(str, item);
        res.push({ step, index });
